...
function addVehicle(model, name, orderprice, saleprice, category, shop, index, length)
    ExecuteSQL(string.format([[
        ...
        print("Zerio-Cardealer-Migration | [" .. index .. "/" .. tostring(length) .. "] " .. name ..
...
function SelectSQL(sqlstr, params, cb) -- Selecting SQL strings
    if GetResourceState("oxmysql") == "started" then -- oxmysql
        local retVal = exports.oxmysql:fetch(sqlstr, params)
        cb(retVal)
    end
    ...
    if GetResourceState("mysql-async") == "started" and GetResourceState("oxmysql") ~= "started" then -- mysql-async
        MySQL.Async.fetchAll(sqlstr, params, function(retVal)
            cb(retVal)
        end)
    end
end
...
function ExecuteSQL(sqlstr, params, cb) -- Executing SQL strings
    if GetResourceState("oxmysql") == "started" then -- oxmysql
        exports.oxmysql:query(sqlstr, params, function(result)
            if cb then
                cb(result)
            end
        end)
    ...
    if GetResourceState("mysql-async") == "started" and GetResourceState("oxmysql") ~= "started" then -- mysql-async
        MySQL.Async.execute(sqlstr, params, function(result)
            if cb then
                cb(result)
            end
        end)
